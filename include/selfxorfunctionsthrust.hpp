/*These two line are intended to prevent the compiler from including this header file twice*/
#ifndef FUNCTIONS_THRUST_H
#define FUNCTIONS_THRUST_H
...
#include <thrust/host_vector.h> /*For Thrust*/
...
void generatePRNG(thrust::host_vector<uint8_t> &random_array,uint32_t alpha,uint32_t manip_sys_time);
uint32_t getSeed(uint8_t lower_bound,uint8_t upper_bound);

/*Self XOR Transform Phase*/
void flattenImage(cv::Mat &image);
void printImageContents(cv::Mat image);
void printVectorCircular(thrust::host_vector <uint8_t> &img_vec,uint16_t xor_position,uint16_t total);
void xorImageEnc(thrust::host_vector<uint8_t> &img_vec,thrust::host_vector<uint8_t> &img_xor_vec,uint32_t m,uint32_t n);
void xorImageDec(thrust::host_vector<uint8_t> &img_vec,thrust::host_vector<uint8_t> &img_xor_vec,uint32_t m,uint32_t n);
