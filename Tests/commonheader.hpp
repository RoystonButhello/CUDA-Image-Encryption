...
namespace common
{
 ...
 static inline void rowColLUTGen(uint32_t *&rowSwapLUT,uint32_t *&rowRandVec,uint32_t *&colSwapLUT,uint32_t *&colRandVec,uint32_t m,uint32_t n);
  ...
  void swappingRandomVecGen(config::lm lm_parameters[],config::lma lma_parameters[],config::slmm slmm_parameters[],config::lasm lasm_parameters[],config::lalm lalm_parameters,config::algorithm rounds[],long double *&time_array,double *x,double *y,double *x_bar,double *y_bar,uint32_t *&row_rotation_vec,uint32_t *&col_rotation_vec,uint32_t *&U,uint32_t *&V,uint32_t row_rotate_seed,uint32_t col_rotate_seed,uint32_t m,uint32_t n,uint32_t total,int iteration);
